...
// @namespace       csgodouble bot fixed by Sakox
// @version         1.00
// @author          Sakox(fix)
...
function Automated() {
            ...
            '<div class="col-lg-9">' +
                ...
                '<div class="form-group automated-hide-on-green">' +
                    '<div class="btn-group">' +
                        ...
                        '<button type="button" class="btn btn-default" id="automated-sako" ' + (this.color === 'sako' ? 'disabled' : '') + '>Sako Method</button>' +
                    '</div>' +
                '</div>' +
            '</div>' +
            '<div class="col-lg-3">' +
    ...
    this.menu = {
        ...
        'sako': document.getElementById('automated-sako'),
        'statistics': {
    ...
    this.menu.sako.onclick = function() {
        self.menu.rainbow.disabled = false;
        self.menu.black.disabled = false;
        self.menu.red.disabled = false;
        self.menu.random.disabled = false;
        self.menu.last.disabled = false;
        self.menu.sako.disabled = true;
        self.color = 'sako';
        self.log('Current mode: sako');
    };
...
Automated.prototype.bet = function(amount, color) {
    ...
    if (color === 'rainbow') {
        ...
        if(this.history[this.history.length - 1] === 'green' && this.history[this.history.length - 2] === 'green' ){
            color = (this.history[this.history.length - 3] === 'red' ? 'black' : 'red');
            this.sakobetted = true;
        } else if(this.history[this.history.length - 1] === 'green'){
            color = (this.history[this.history.length - 2] === 'red' ? 'black' : 'red');
            this.sakobetted = true;
        } else if(this.history[this.history.length - 2] === 'green'){
            color = (this.history[this.history.length - 1] === 'red' ? 'black' : 'red');
            this.sakobetted = true;
        }else if (this.history[this.history.length - 1] === this.history[this.history.length - 2]) {
            color = (this.history[this.history.length - 1] === 'red' ? 'black' : 'red');
            this.sakobetted = true;
        } else {
            this.log('Not betting!');
            this.last_result = 'invalid bet';
            this.waiting_for_bet = false;
            this.sakobetted = false;
            return true;
        }
    }
...
Automated.prototype.play = function() {
    ...
    this.game = setInterval(function() {
        ...
        if (!self.waiting_for_bet && self.updateAll() && !history.equals(self.history)) {
            ...
            if (self.last_color === null || !self.sakobetted) {
